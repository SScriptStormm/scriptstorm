-- CRITICAL SECURITY FIXES: Address all identified vulnerabilities (CORRECTED)
...
-- 5. Security Monitoring: Add comprehensive access logging for INSERT/UPDATE/DELETE
CREATE OR REPLACE FUNCTION public.log_security_access()
RETURNS TRIGGER AS $$
BEGIN
    -- Log all data modification attempts to sensitive tables
...
-- Apply security logging to sensitive tables (only for data modifications)
...
CREATE TRIGGER security_log_edge_tokens
    AFTER INSERT OR UPDATE OR DELETE ON public.edge_function_tokens  
    FOR EACH ROW EXECUTE FUNCTION public.log_security_access();
