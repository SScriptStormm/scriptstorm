...
import { CheckCircle, Star, FileText, Package, Clock, RefreshCw, Search, Shield, Headphones, TrendingUp, BarChart3, Map, Infinity, Instagram } from "lucide-react";
...
const EnterprisePackageCard = ({
  ...
  return <Card className={`relative border-0 hover:scale-105 transition-all duration-500 group flex flex-col ${pkg.badge ? 'mt-6 md:mt-0' : ''}`} style={{
    ...
    border: `2px solid ${pkg.color}40`
      ...
      {pkg.badge && <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-50">
          <Badge className="px-4 py-2 text-sm font-bold text-white shadow-lg" style={{
      ...
      <CardContent className="space-y-2 md:space-y-6 p-3 md:p-6 relative flex-1 flex flex-col">
        <div className="space-y-1 md:space-y-2 flex-1">
          ...
          return <>
              ...
              return <div key={`${pkg.id}-base-${index}`} className="flex items-start gap-2">
                ...
                const [boldPart, ...restParts] = feature.split(':');
                const restText = restParts.join(':');
                return <div key={`${pkg.id}-expanded-${index + 5}`} className="flex items-start gap-2">
                          <div className="w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{
                    backgroundColor: pkg.color
                  }}>
                            {getFeatureIcon(feature)}
        ...
        {pkg.note && <p className="text-xs text-muted-foreground italic">{pkg.note}</p>}

        {pkg.details && isExpanded && <p className="text-xs text-muted-foreground bg-muted/30 p-2 rounded">{pkg.details}</p>}

        {(() => {
        const displayFeatures = isAnnual && pkg.annualFeatures ? pkg.annualFeatures : pkg.features;
        return displayFeatures.length > 5 && <Button variant="ghost" size="sm" onClick={toggleExpansion} className="text-xs w-full relative hover:bg-opacity-10" style={{
          color: pkg.color,
        ...
        <Button onClick={() => onCheckout(pkg.id)} disabled={loadingStates[pkg.id]} className="w-full font-bold py-2 md:py-3 px-4 md:px-6 text-sm md:text-base transition-all duration-300" style={{
        background: `linear-gradient(135deg, ${pkg.color}, ${pkg.color}dd)`,
        color: 'white'
      }}>
          {loadingStates[pkg.id] ? "Processing..." : isAnnual ? pkg.id === 'dominance' ? "💎 Start Saving & Get My 12-Hour Draft" : "💎 Start Saving & Get My 24-Hour Draft" : pkg.id === 'dominance' ? "🚀 Start My 12-Hour Draft" : "🚀 Start My 24-Hour Draft"}
        </Button>
        
        <a href="https://docs.google.com/document/d/11oF_e-yAGsdnYLenzUswEZtOaWbIb_04uFQWGuPS4pk/edit?usp=sharing" target="_blank" rel="noopener noreferrer" className="text-xs md:text-sm text-center block hover:underline transition-all duration-200" style={{
        color: pkg.color
      }}>
